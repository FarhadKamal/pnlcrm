...
<button onclick="window.print()" class="btn btn-sm btn-darkblue printBtn">Print Quotation</button>
<div class="quotDiv" id="section-to-print">
    ...
    <div>
        <div class="quotContainer headerContainer">
        ...
        <div>
            ...
            {{-- <p>{{ $leadInfo['location']->location_name }}</p> --}}
            ...
            <p style="margin-top: 2%;">Subject :<span class="boldText"><u>Price Quotation for the supply of pump
                        motor</u></span>
            </p>

            <p class="boldText" style="margin-top: 2%">Greetings,</p>
            <p>Thank you for your enquiry and reference is made to your requirement of pump. We are pleased to submit
                        ...
                        <tbody>
                            @foreach ($pumpInfo as $itemPump)
                                @if ($itemPump->req_id == $itemReq->id)
                                    <tr class="tableRow">
                                        <td class="table1RowCol1">
                                            <p class="colText">{{ $itemPump['productInfo']->brand_name }}</p>
                                        </td>
                                        <td class="table1RowCol1">
                                        ...
                                        <td class="table1RowCol1">
                                            <p class="text-center colText">HP: {{ $itemPump['productInfo']->hp }}, KW:
                                                {{ $itemPump['productInfo']->kw }}</p>
                                        </td>
                                        <td class="table1RowCol1">
                                            <p class="text-center colText">{{ $itemPump['productInfo']->head }}</p>
                                        </td>
                                        <td class="table1RowCol1">
                                            <p class="text-end colText">
                                                {{ number_format((float) $itemPump->unit_price, 2, '.', ',') }}</p>
                                        </td>
                                        <td class="table1RowCol1">
                                        ...
                                        <td class="table1RowCol1">
                                            <p class="colText text-end">
                                                {{ number_format((float) $itemPump->discount_price, 2, '.', ',') }}</p>
                                        </td>
                                        <td class="table1RowCol1">
                                            <p class="colText text-end">
                                                {{ number_format((float) $itemPump->net_price, 2, '.', ',') }}</p>
                                        </td>
                                    </tr>
                                @endif
                            @endforeach
        ...
        <div class="pagebreakAvoid">
            ...
            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i></i> Payment shall be made
                through crossed cheque
                within 30 days and Purchase Order in favor of PNL
                Holdings Limited.</p>
            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i> Provide your concern
                Factory address & BIN
                number for issue the Vat Challan (Mushak-6.3).</p>
            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i> Delivery after 5 days from
                the date of your
                confirmed Purchase Order subject to available in our
                stock.</p>
            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i> Delivery from Pedrollo
                Plaza, 5, Jubilee road,
                Chittagong.</p>

            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i> 2 (Two) Years’ Service
                Warranty for Pump-moto.
            </p>

            <p class=""><i class="fa-regular fa-circle-dot" style="font-size:8px"></i> Price offer validity 7 days
                from the date
                hereof.</p>
            <br>
